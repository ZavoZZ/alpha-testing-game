...
class MarketplaceService {
    ...
    static async browseListings({ category, quality, minPrice, maxPrice, sortBy = 'listed_at', page = 1, limit = 20 }) {
        ...
        // NOTE: Decimal128 fields must be converted to double for $multiply
        pipeline.push({
            $addFields: {
                price_with_vat: {
                    $multiply: [{ $toDouble: '$price_per_unit_euro' }, 1.10]
                },
                total_with_vat: {
                    $multiply: [{ $toDouble: '$total_value_euro' }, 1.10]
                }
            }
        });
        ...
        return {
            listings: listings.map(listing => {
                // After aggregation, Decimal128 fields are converted to strings or numbers
                // Handle both cases safely
                const pricePerUnit = listing.price_per_unit_euro?.$numberDecimal 
                    || listing.price_per_unit_euro?.toString() 
                    || String(listing.price_per_unit_euro || '0');
                const totalValue = listing.total_value_euro?.$numberDecimal 
                    || listing.total_value_euro?.toString() 
                    || String(listing.total_value_euro || '0');
                const qty = listing.quantity?.$numberDecimal 
                    || listing.quantity?.toString() 
                    || String(listing.quantity || '0');
                const priceWithVat = listing.price_with_vat 
                    ? String(listing.price_with_vat) 
                    : '0';
                const totalWithVat = listing.total_with_vat 
                    ? String(listing.total_with_vat) 
                    : '0';
                
                return {
                    ...listing,
                    price_per_unit_euro: pricePerUnit,
                    total_value_euro: totalValue,
                    quantity: qty,
                    price_with_vat: FinancialMath.round(priceWithVat, 4),
                    total_with_vat: FinancialMath.round(totalWithVat, 4)
                };
            }),
            pagination: {
                page,
