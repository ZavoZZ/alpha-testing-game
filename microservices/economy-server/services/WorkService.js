...
class WorkService {
	...
	static async processWorkShift(userId) {
		...
		if (masterUser && FinancialMath.isGreaterThan(masterTaxRounded, '0.0000')) {
			...
			// Get user balance before for ledger
			const userBalanceBefore = FinancialMath.toString(user.balance_euro);
			const companyBalanceBefore = FinancialMath.toString(company.funds_euro);
			
			// Ledger: Company → Worker (gross payment)
			await Ledger.create([{
				transaction_type: 'WORK',
				sender: company.owner_id,  // Company owner
				receiver: user._id,
				currency: 'EURO',
				amount_gross: grossSalaryRounded,
				amount_tax: governmentTaxRounded,
				amount_net: netSalaryRounded,
				tax_rate: WORK.INCOME_TAX_PERCENTAGE,
				sender_balance_before: companyBalanceBefore,
				sender_balance_after: FinancialMath.toString(company.funds_euro),
				receiver_balance_before: userBalanceBefore,
				receiver_balance_after: FinancialMath.add(userBalanceBefore, netSalaryRounded),
			...
			await Ledger.create([{
				...
				sender: user._id,
				receiver: null,  // Treasury (system)
				currency: 'EURO',
				amount_gross: governmentNetTax,
				amount_tax: '0.0000',
				amount_net: governmentNetTax,
				tax_rate: '0.0000',
				sender_balance_before: FinancialMath.add(userBalanceBefore, netSalaryRounded),
				sender_balance_after: FinancialMath.add(userBalanceBefore, netSalaryRounded),
				receiver_balance_before: '0.0000',
				receiver_balance_after: '0.0000',
		...
		if (masterUser && FinancialMath.isGreaterThan(masterTaxRounded, '0.0000')) {
			await Ledger.create([{
				transaction_type: 'REFERRAL_BONUS',
					sender: user._id,
					receiver: masterUser._id,
					currency: 'EURO',
					amount_gross: masterTaxRounded,
					amount_tax: '0.0000',
					amount_net: masterTaxRounded,
					tax_rate: '0.0000',
					sender_balance_before: FinancialMath.add(userBalanceBefore, netSalaryRounded),
					sender_balance_after: FinancialMath.add(userBalanceBefore, netSalaryRounded),
					receiver_balance_before: FinancialMath.toString(masterUser.balance_euro),
					receiver_balance_after: FinancialMath.add(FinancialMath.toString(masterUser.balance_euro), masterTaxRounded),
