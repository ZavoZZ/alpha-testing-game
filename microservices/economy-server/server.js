...
userSchema.index({
	is_frozen_for_fraud: 1,
	vacation_mode: 1,
	energy: 1,
	happiness: 1,
}); // Compound index for entropy query
...
treasurySchema.statics.collectTax = async function (
	taxType,
	currency,
	amount,
	session = null,
	...
	if (treasury[taxField]) {
		...
		const newAmount = (
			parseFloat(current) + parseFloat(amountDecimal.toString())
		).toFixed(4);
	...
	const newTotal = (
		parseFloat(currentTotal) + parseFloat(amountDecimal.toString())
	).toFixed(4);
...
ledgerSchema.statics.createTransaction = async function (data, session = null) {
	...
	const lastTransaction = await Ledger.findOne()
		.sort({ createdAt: -1 })
		.session(session);
	const previous_hash = lastTransaction
