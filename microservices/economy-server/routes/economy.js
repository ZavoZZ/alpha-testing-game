...
router.get('/admin/treasury', async (req, res) => {
	try {
		...
		res.json({
			success: true,
			data: {
				collected_work_tax_euro: FinancialMath.toString(treasury.collected_work_tax_euro),
				collected_market_tax_euro: FinancialMath.toString(treasury.collected_market_tax_euro),
				collected_transfer_tax_euro: FinancialMath.toString(treasury.collected_transfer_tax_euro),
				total_collected: FinancialMath.toString(treasury.total_collected),
				updated_at: treasury.updatedAt
			}
		});
		
	} catch (error) {
		console.error('[API] ❌ Get treasury error:', error);
		
		res.status(500).json({
			success: false,
			error: 'Failed to retrieve treasury',
			message: error.message
		});
	}
});
...
router.get('/admin/users', async (req, res) => {
	try {
		...
		const users = await User.find({}).select('-password -refresh_token').lean();
		...
		res.json({
			...
			data: users.map(user => ({
				...
				email: user.email,
				role: user.role,
				balance_euro: FinancialMath.toString(user.balance_euro),
				balance_gold: FinancialMath.toString(user.balance_gold),
				balance_ron: FinancialMath.toString(user.balance_ron),
				energy: user.energy,
				happiness: user.happiness,
				is_frozen_for_fraud: user.is_frozen_for_fraud,
	...
	try {
		...
		res.json({
			...
			data: companies.map(company => ({
				...
				name: company.name,
				type: company.type,
				funds_euro: FinancialMath.toString(company.funds_euro),
				wage_offer: FinancialMath.toString(company.wage_offer),
				employees_count: company.employees ? company.employees.length : 0,
				max_employees: company.max_employees,
				is_hiring: company.is_hiring,
				is_government: company.is_government,
				status: company.status,
...
router.get('/admin/marketplace', async (req, res) => {
	try {
		...
		const listings = await MarketplaceListing.find({ status: 'ACTIVE' }).lean();
		...
		res.json({
			...
			data: listings.map(listing => ({
				...
				quantity: FinancialMath.toString(listing.quantity),
				price_per_unit_euro: FinancialMath.toString(listing.price_per_unit_euro),
...
console.log('[Economy Routes] 👑 Admin endpoints active');
