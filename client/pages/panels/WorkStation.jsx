	...
	useEffect(() => {
		...
		const updateCountdown = () => {
			...
			const nextAvailable = new Date(
				workStatus.cooldown.next_available_at,
			).getTime();
		...
		return (
			<div className="glass-container animate-fade-in" style={styles.container}>
				...
				<div style={styles.skeleton}>
					...
					<div
						className="skeleton-line"
						style={{ width: '40%', height: '20px', marginBottom: '24px' }}
					></div>
					<div
						className="skeleton-line"
		...
		<div className="glass-container animate-fade-in" style={styles.container}>
			...
			<div style={styles.statsGrid}>
				<div style={styles.statBox}>
					...
					<div
						style={{ ...styles.statValue, color: getStatColor(player.energy) }}
					>
						{player.energy}/100
					</div>
				</div>
				<div style={styles.statBox}>
					...
					<div
						style={{
							...styles.statValue,
							color: getStatColor(player.happiness),
						}}
					>
				...
				<div style={styles.statBox}>
					...
					<div
						style={{ ...styles.statValue, color: getStatColor(player.health) }}
					>
						{player.health}/100
					</div>
				</div>
					...
					{warnings.map((warning, idx) => (
						<div
							...
							style={{
								...styles.warningBox,
								borderColor:
									warning.severity === 'critical' ? '#ff4444' : '#ffaa00',
							}}
						>
				...
				{cooldownActive ? (
					...
					>
						{blockedReason === 'INSUFFICIENT_ENERGY'
							? '⚡ Insufficient Energy'
							: blockedReason === 'COMPANY_INSOLVENT'
								? '💸 Company Insolvent'
								: blockedReason === 'DEAD'
									? '💀 Cannot Work (Dead)'
									: blockedReason === 'VACATION_MODE'
			...
			{company && !company.can_afford_salary && (
				<div style={styles.insolvencyWarning}>
					...
					<span style={styles.insolvencyText}>
						<strong>Company Insolvent!</strong> {company.name} has only €
						{company.funds} remaining. They cannot afford to pay your salary.
						Contact the owner!
					</span>
				</div>
			)}
			...
			{player && player.total_shifts_worked > 0 && (
				<div style={styles.statsFooter}>
					...
					<div style={styles.statItem}>
						...
						<span style={{ ...styles.statValue, color: '#44ff88' }}>
							€{player.balance}
						</span>
					</div>
				</div>
...
const styles = {
	...
	paycheckPreview: {
		background:
			'linear-gradient(135deg, rgba(68, 255, 136, 0.1), rgba(68, 136, 255, 0.1))',
		borderRadius: '16px',
		padding: '24px',
		marginBottom: '24px',
	...
	paycheckDivider: {
		height: '2px',
		background:
			'linear-gradient(90deg, transparent, rgba(68, 255, 136, 0.5), transparent)',
		margin: '16px 0',
	},
