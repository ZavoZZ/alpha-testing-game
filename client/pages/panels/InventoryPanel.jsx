    ...
    const fetchInventory = async () => {
        ...
        try {
            ...
            if (data.success) {
                ...
                setError(data.message || 'Eroare la încărcarea inventarului');
            }
        } catch (err) {
...
function ConsumptionModal({ item, onClose, onSuccess }) {
    ...
    const effects = item.calculated_effects || {};
    const energyAfter = Math.min(100, (user?.energy || 0) + (effects.energy_restore || 0) * quantity);
    const happinessAfter = Math.min(100, (user?.happiness || 0) + (effects.happiness_restore || 0) * quantity);
    
    return (
        <div className="modal-overlay" onClick={onClose}>
            <div className="modal-content consumption-modal" onClick={e => e.stopPropagation()}>
                ...
                <div className="effect-preview">
                    ...
                    {effects.energy_restore > 0 && (
                        <div className="effect">
                            ⚡ Energie: {user?.energy || 0} → {energyAfter} (+{effects.energy_restore * quantity})
                        </div>
                    )}
                    {effects.happiness_restore > 0 && (
                        <div className="effect">
                            😊 Fericire: {user?.happiness || 0} → {happinessAfter} (+{effects.happiness_restore * quantity})
                        </div>
                    )}
                </div>
